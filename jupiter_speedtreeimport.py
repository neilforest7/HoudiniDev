...
import hou
# from python.jupiter_speedtreepost import SpeedTreePostAction
...
class SpeedTreeMaterialCreatorPrincipled(SpeedTreeMaterialCreator):
    ...
    @staticmethod
    def CanCreate():
        return hou.nodeType("Vop/SpeedTreePrincipled") is not None
    ...
    def CreateMaterial(self, stMaterial):
        ...
        if "Specular" in stMaterial.maps:
        ...
        if "Metallic" in stMaterial.maps:
...
class SpeedTreeMaterialCreatorMantra(SpeedTreeMaterialCreator):
    ...
    @staticmethod
    def CanCreate():
        return hou.nodeType("Vop/SpeedTreeMantra") is not None
...
class SpeedTreeMaterialCreatorRenderman(SpeedTreeMaterialCreator):
    ...
    @staticmethod
    def CanCreate():
        return hou.nodeType("Vop/pxrmaterialbuilder") is not None
...
################################################################
# SpeedTreeMaterialCreatorArnold

# class SpeedTreeMaterialCreatorArnold(SpeedTreeMaterialCreator):
#     description = "SpeedTree Arnold"
#
#     @staticmethod
#     def CanCreate():
#         return hou.nodeType("Vop/pxrmaterialbuilder") != None
#
#     def MakeTexture(self, stMap, name, parent, linear):
#         tex = parent.createNode("pxrtexture")
#         tex.setName(name)
...
class SpeedTreeMaterialCreatorVray(SpeedTreeMaterialCreator):
    ...
    @staticmethod
    def CanCreate():
        return hou.nodeType("Vop/vray_vop_material") is not None
    ...
    def CreateMaterial(self, stMaterial):
        ...
        if (twoSidedNode is not None) and (
                ("SubsurfaceColor" in stMaterial.maps) or ("SubsurfaceAmount" in stMaterial.maps)):
...
class SpeedTreeMaterialCreatorRedshift(SpeedTreeMaterialCreator):
    ...
    @staticmethod
    def CanCreate():
        return hou.nodeType("Vop/redshift_vopnet") is not None
...
################################################################
# LoadSpeedTree
def LoadSpeedTree(hdanode, stmatFile, materialCreatorFunc, position):
    stmatFile = os.path.realpath(stmatFile).replace("\\", "/")
    print("stmatFile = " + stmatFile)
    stmatPath = os.path.dirname(stmatFile).replace("\\", "/") + "/"
    print("stmatPath = " + stmatPath)
    try:
        ...
        hdanode.parm('treename').set(mesh)
    ...
    try:
        ...
        if extension == ".obj":
            ...
            if materialCreatorFunc is not None:
                try:
                ...
                while aNodeStack:
                    ...
                    if node.type() == alembicType:
                        ...
                        for index, group in enumerate(primGroups, 1):
                            ...
                            matNode.parm("shop_materialpath" + str(index)).set(
                                matNode.relativePathTo(aNewMaterials[matName].shader))

                    for child in node.children():
                        aNodeStack.append(child)

        elif extension == ".fbx":
            print("extension is fbx")
            # handle FBX files
            ...
            while aNodeStack:
                ...
                if node.type() == shopType:
                    ...
                    for child in node.children():
                        aNodeStack.append(child)
                        # print("aNodeStack =====", str(aNodeStack))
                    hdanode.parm('sop_path').set(node.path())
            hdanode.parm('sopmat_path').set(aSopNodes[0].path())
            ...
            if materialCreatorFunc is not None:
                ...
                hdanode.parm('matnet_path').set(nodeMaterialsNew.path())
...
################################################################
# LoadSpeedTreeGUI
def LoadSpeedTreeGUI(hdanode=None):
    # ask for filename
    ...
    if not stmatFile:
        ...
        aMaterialCreatorClasses = []
        for subclass in SpeedTreeMaterialCreator.__subclasses__():
            if subclass.CanCreate():
                aMaterialCreatorNames.append(subclass.description)
                aMaterialCreatorClasses.append(subclass)
        aMaterialCreatorNames.append("No change")
        materialType = hou.ui.selectFromList(aMaterialCreatorNames, [0], True,
                                             "Choose the type of shader to use for the SpeedTree materials",
                                             "Materials")
        if not materialType:
            print("SpeedTree import cancelled")
        else:
            materialCreatorFunc = None
            if materialType[0] < len(aMaterialCreatorClasses):
                materialCreatorFunc = aMaterialCreatorClasses[materialType[0]]().CreateMaterial

            position = [0, 0, 0]
            if hdanode is not None:
                position = hdanode.parmTuple('t').eval()
            LoadSpeedTree(hdanode, stmatFile, materialCreatorFunc, position)

            # post action to smooth operation
            installed_node_color = hou.Color((0.054, 0.325, 0.603))
            hdanode.setDisplayFlag(False)
            hdanode.setGenericFlag(hou.nodeFlag.Selectable, False)
            hdanode.setColor(installed_node_color)
            hdanode.setUserData('nodeshape', 'star')
