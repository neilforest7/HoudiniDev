...
def uninstall(node):
    ...
    result = hou.ui.displayMessage(noti, buttons=('Yes, Delete', 'Cancel'), title='Jupiter Uninstall')
    if result == 0:
        ...
        for files in os.listdir(otls_path):
            ...
            if os.path.isfile(otl):
                hou.hda.uninstallFile(otl, oplibraries_file='Scanned Asset Library Directories')
        # Remove Embedded HDA
